...
- (void)addPostValue:(id <NSObject>)value forKey:(NSString *)key
{
  ...
  //  NSLog(@"self postData %@",[self postData]);
}
 ...
 *  签名生成算法
 *
 *  @param params 请求参数数组集，所有参数必须已转换为字符串类型
 *
 *  @return 签名
 */

- (NSString *)getSignature:(NSArray *)postDataArray
{
    
    
    /*aes加密+base64后(key:2015-05-01 05:10:11)*/
    //NSString *secretYKey = @"vKHTPGTfhBPqrFBDVYDkv8uXfFpH3ixy";(原KEY)
//    NSData *plain = [secretYKey dataUsingEncoding:NSUTF8StringEncoding];
//    NSData *cipher = [plain AES256EncryptWithKey:@"2015-05-01 05:10:11"];
//    NSLog(@"cipher   %@",[[NSString alloc] initWithData:[GTMBase64 encodeData:cipher]encoding:NSUTF8StringEncoding]);
    
    
    // 签名密钥
    //解密Key
    NSString *secretEncodeKey = @"vNP6dZvHQFZhxWjJmcxXUR7CYQipcaYqssDVyUEJ96VqhuzRU3IRMOTZ+cNqampE";//aes加密+base64后(key:2015-05-01 05:10:11)
    NSString *strKeyfordecode = @"2015-05-01 05:10:11";
    NSData* aesData = [GTMBase64_WD decodeString:secretEncodeKey];
    NSData* keyData = [aesData AES256DecryptWithKey:strKeyfordecode];
    NSString *secretKey = [[NSString alloc] initWithData:keyData encoding:NSUTF8StringEncoding];
  //  NSLog(@"secretkey %@",secretKey);
    
    // 把postDataArray中的参数名和参数值以key/value的形式存储在字典中
    NSMutableDictionary *paramsDic = [[NSMutableDictionary alloc] init];
    for (NSDictionary *val in postDataArray) {
        [paramsDic setValue:val[@"value"] forKey:val[@"key"]];
    }
    
    // 先将参数以其参数名的字典序升序进行排序
    NSArray *sortedKeys = [paramsDic.allKeys sortedArrayUsingComparator:^NSComparisonResult(id a, id b) {
        NSString *first = a;
        NSString *second = b;
    ...
    for (NSString *key in sortedKeys) {
        [basestring appendFormat:@"%@=%@", key, paramsDic[key]];
    }
    ...
    NSString *result = [basestring MD5];
...
#ifdef SECRET_REQUEST
    [self appendPostString:[NSString stringWithFormat:@"&%@=%@", [self encodeURL:@"sign"], [self encodeURL:[self getSignature:[self postData]]]]];
#endif
