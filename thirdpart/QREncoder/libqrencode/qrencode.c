...
static void RSblock_initBlock(RSblock *block, long dl, unsigned char *data, long el, unsigned char *ecc, RS *rs)
{
	block->dataLength = dl;
	block->data = data;
	block->eccLength = el;
...
static long RSblock_init(RSblock *blocks, long spec[5], unsigned char *data, unsigned char *ecc)
{
	int i;
	RSblock *block;
	unsigned char *dp, *ep;
...
static FrameFiller *FrameFiller_new(long width, unsigned char *frame)
{
	FrameFiller *filler;
...
static QRcode *QRcode_new(long version, long width, unsigned char *data)
{
	QRcode *qrcode;
